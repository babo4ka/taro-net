...
descs = load_descs('present', path='../../cards_descs')
...
class PresentNet(nn.Module):
    def __init__(self, input_size, hidden_size, embedding_size, n_layers=1):
        super(PresentNet, self).__init__()
...
net = PresentNet(input_size=len(indexes_to_words), hidden_size=128, embedding_size=256, n_layers=3)
net.to(device)
...
for epoch in range(epochs):
    ...
    if epoch % 100 == 0:
        ...
        plt.plot(loss_history, c='pink', label='потери сети настоящего')
        plt.plot(gen_loss_epochs, gen_loss_history, marker=2, c='black', label='потери на поколении')
        plt.xlabel('эпохи')
        plt.ylabel('потери')
        plt.legend(loc='upper left')
        plt.show()

        torch.save(net, ("../learned_nets/present/PresentNet_temp_" + str(temp_num) + ".pt"))
        temp_num += 1

        predicted_text = generate_text(net, words_to_indexes, indexes_to_words, device, start_text='пpи гaдaнии')
...
torch.save(net, "../learned_nets/present/PresentNet.pt")
